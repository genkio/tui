// Package readstore persists which bilibili 动态 posts the user has marked read.
// bilibili exposes no read state for the timeline, so bilibili-tui tracks it
// locally: a set of dynamic ids saved as JSON under the user's state dir and
// re-applied on every fetch, so a post you already saw stays greyed (or hidden)
...
// without bound. The timeline only surfaces recent posts, so once an id ages out
// of the newest maxEntries it will never resurface and is safe to forget.
const maxEntries = 20000

// Store is a persistent set of read dynamic ids. It is safe for concurrent use:
// the UI marks and queries from the update loop while Save runs on a background
// goroutine.
type Store struct {
	path string
	mu   sync.Mutex
	ids  map[string]int64 // dynamic id -> unix seconds when marked read
}
...
// DefaultPath is where read state persists:
// $TUI_STATE_DIR/state/bilibili-tui/read.json when a shared state dir is set
// (synced between devices), else $XDG_STATE_HOME/bilibili-tui/read.json, else
// ~/.local/state/bilibili-tui/read.json.
func DefaultPath() string {
	return core.StatePath("bilibili-tui", "read.json")
}
