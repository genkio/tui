...
func fetchFeed(ctx context.Context, c *bilibili.Client, cfg config.Config, reset bool) tea.Cmd {
	return func() tea.Msg {
		videos, err := c.Feed(ctx, cfg.MaxVideos)
		if err != nil {
			return errMsg{err}
		}
		return feedMsg{videos: videos, reset: reset}
	}
}

// scheduleRefresh emits an autoRefreshMsg after d; the update loop reschedules it
// to form a recurring timer.
