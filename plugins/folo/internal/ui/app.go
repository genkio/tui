...
// pending (unread) Folo articles that expand inline, plus mark-as-read.
package ui
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case contentMsg:
		if msg.err != nil {
			m.feed.setContentErr(msg.id, friendlyError(msg.err))
		} else {
			m.feed.setContent(msg.id, msg.text)
		}
		return m, nil
...
func (m Model) handleKey(msg tea.KeyPressMsg) (tea.Model, tea.Cmd) {
	...
	case key.Matches(msg, m.keys.Expand):
		...
		if m.feed.needsContent(a.ID) {
			m.feed.startContent(a.ID)
			cmds = append(cmds, fetchContent(m.ctx, m.client, a.ID))
		}
		// Expanding also marks read, unless the article is pinned (K).
		if !m.feed.isRead(a.ID) && !m.feed.isKept(a.ID) {
			m.feed.markReadLocal(a.ID)
			cmds = append(cmds, markRead(m.ctx, m.client, a.ID))
		}
		return m, tea.Batch(cmds...)
...
func (m Model) headerView() string {
	left := headerStyle.Render("folo-tui")
	label := "Articles"
	if m.cfg.UnreadOnly {
		label = "Pending"
	}
...
func (m Model) bodyView() string {
	...
	case len(m.feed.articles) == 0:
		return center(emptyStyle.Render("All caught up. Nothing pending."), m.width, h)
	default:
		return m.feed.View()
	}
}
