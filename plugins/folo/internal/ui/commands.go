...
func fetchUnreads(ctx context.Context, c *folo.Client, unreadOnly bool, max int, reset bool) tea.Cmd {
	return func() tea.Msg {
		...
		return articlesMsg{items: folo.ToItems(arts), reset: reset}
	}
}

// fetchContent loads one entry's body on demand; the list response omits it, so
// this runs when an article is first expanded.
func fetchContent(ctx context.Context, c *folo.Client, id string) tea.Cmd {
	return func() tea.Msg {
		text, err := c.Content(ctx, id)
		return contentMsg{id: id, text: text, err: err}
	}
}
...
func markRead(ctx context.Context, c *folo.Client, id string) tea.Cmd {
	return func() tea.Msg {
...
func markUnread(ctx context.Context, c *folo.Client, id string) tea.Cmd {
	return func() tea.Msg {
