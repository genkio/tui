...
// Source adapts the Inoreader client to core.Source, so the standalone UI, the
// --json/--count/--mark-read commands, and the launcher's merged view all read
// unread articles the same way.
type Source struct {
	client     *Client
	unreadOnly bool
	max        int
...
// ToItems normalizes articles into core.Items. Inoreader gives no absolute
// publish time (only a relative age), so At is left zero and the merged view
// sorts these from Age.
