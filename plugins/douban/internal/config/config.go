...
// DefaultPath is $XDG_CONFIG_HOME/douban-tui/config.toml, falling back to
// ~/.config/douban-tui/config.toml.
func DefaultPath() string {
	if dir := os.Getenv("XDG_CONFIG_HOME"); dir != "" {
		return filepath.Join(dir, "douban-tui", "config.toml")
	}
	home, err := os.UserHomeDir()
	if err != nil {
		return ""
	}
	return filepath.Join(home, ".config", "douban-tui", "config.toml")
}
