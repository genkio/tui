// Package readstore persists which Douban statuses the user has marked read.
// Douban exposes no read state for the timeline, so douban-tui tracks it
// locally: a set of status ids saved as JSON under the user's state dir and
// re-applied on every fetch, so a status you already saw stays greyed (or
...
// maxEntries caps how many read markers we retain so the file can't grow
// without bound. The timeline only surfaces recent statuses, so once an id
// ages out of the newest maxEntries it will never resurface and is safe to
// forget.
const maxEntries = 20000

// Store is a persistent set of read status ids. It is safe for concurrent use:
// the UI marks and queries from the update loop while Save runs on a background
// goroutine.
type Store struct {
	path string
	mu   sync.Mutex
	ids  map[string]int64 // status id -> unix seconds when marked read
}
...
// DefaultPath is $XDG_STATE_HOME/douban-tui/read.json, falling back to
// ~/.local/state/douban-tui/read.json. Empty when the home dir can't be resolved.
func DefaultPath() string {
	if dir := os.Getenv("XDG_STATE_HOME"); dir != "" {
		return filepath.Join(dir, "douban-tui", "read.json")
	}
	home, err := os.UserHomeDir()
	if err != nil {
		return ""
	}
	return filepath.Join(home, ".local", "state", "douban-tui", "read.json")
}
...
// Save writes the store to disk atomically (temp file + rename) after pruning
// to the newest maxEntries markers. A store with no resolvable path is a silent
// no-op.
