...
// SyncDir is the optional single root for everything worth syncing between
...
// read marks across machines. Read from $TUI_SYNC_DIR (the launcher's
// --sync-dir exports it), with a leading ~ expanded and the result made
...
func SyncDir() string {
	dir := strings.TrimSpace(os.Getenv("TUI_SYNC_DIR"))
	if dir == "" {
		return ""
	}
...
// $TUI_SYNC_DIR/state/<app>/<file> when the sync dir is set, else
...
// $TUI_SYNC_DIR/config/<app>/<file> when the sync dir is set, else
