...
// (Helium, Chromium, Brave, Chrome, Edge, Vivaldi, Arc), or an error naming what
// to install. Any Chromium browser speaks the DevTools protocol; WebKit (Safari)
// does not, so a Safari-only user sets credentials by hand instead.
//
// The ungoogled builds (Helium, then Chromium) are tried first so they are the
// preferred login browser over the branded ones; reorder the lists below to
// change the preference.
func FindChromium() (string, error) {
	if runtime.GOOS == "darwin" {
		for _, app := range []string{"Helium", "Chromium", "Brave Browser", "Google Chrome", "Microsoft Edge", "Vivaldi", "Arc"} {
		...
		for _, name := range []string{"helium", "chromium", "chromium-browser", "brave-browser", "brave", "google-chrome", "google-chrome-stable", "microsoft-edge", "vivaldi-stable"} {
			if p, err := exec.LookPath(name); err == nil {
				return p, nil
			}
		}
	}
	return "", errors.New("no Chromium-family browser found (Helium, Chromium, Brave, Chrome, Edge, …); install one, or set credentials by hand in " + UserEnvPath())
}
