...
// feedModel renders the unread articles as a single scrolling list (oldest at
// the top). A cursor selects one article; expanding it shows the full body
// inline, accordion-style. The viewport is positioned to keep the cursor
// visible.
...
func newFeed() feedModel {
	return feedModel{expanded: map[string]bool{}, read: map[string]bool{}, kept: map[string]bool{}, showFeed: true, vp: viewport.New()}
}
...
// resetCursor jumps back to the oldest item, which a manual refresh wants but a
// background auto-refresh does not.
func (f *feedModel) setArticles(a []inoreader.Article, resetCursor bool) {
	f.articles = a
...
func (f feedModel) selectedArticle() (inoreader.Article, bool) {
	if f.cursor < 0 || f.cursor >= len(f.articles) {
		return inoreader.Article{}, false
	}
...
func (f feedModel) renderArticle(a inoreader.Article, selected, expanded, read bool, width int) []string {
	gutter := "  "
	if selected {
	...
	title := titleSt.Render(truncate(a.Title, avail))
